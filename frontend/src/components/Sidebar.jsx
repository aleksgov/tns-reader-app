...
export default function Sidebar({ isMenuOpen, setIsMenuOpen }) {
    return (
        <div className={`bg-[#2b2b2b] backdrop-blur-sm flex flex-col pb-4 transition-all duration-300 overflow-hidden ${isMenuOpen ? 'w-72' : 'w-11'}`}>
            {/* Иконка меню */}
            <div className="flex items-center h-12 w-full">
                ...
                >
                    <Menu className="w-5 h-5 text-white hover:text-white flex-shrink-0" />
            ...
            <div className="flex items-center h-12 mt-auto w-full">
                <button className="flex items-center w-full h-full px-4 justify-start gap-2">
                    <Settings className="w-4 h-4 text-white hover:text-white flex-shrink-0" />
